...
API_KEY = os.environ.get("API_KEY", "psychoxd")   # fallback agar env nahi hai
...
def root():
    return {
        ...
        "docs": "/docs",
        "developer": "@psychopathmc",   # 🔥 Changed
    }
...
async def search(
    ...
    key: str = Query(..., description="API Key required"),   # 🔥 Key mandatory
):
    # 🔥 Check API key
    if key != API_KEY:
...
async def search_parallel(req: BatchRequest):
    # Key check for parallel endpoint? But we can skip for simplicity, or add a key param in body.
    # For now, we'll also require key in query for parallel.
    # But to keep it simple, we'll add key as query param to this endpoint too.
    # However, the user may not use this, so we can leave it.
    if not req.queries:
...
def build_ui():
    with gr.Blocks(
        ...
        gr.Markdown("# 🔍 PSYCHO x Info", elem_classes="main-title")
